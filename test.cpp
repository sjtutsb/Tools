//test.cpp for triplet loss  --sibin
...
int main(){
	...
	for (int i = 0; i < size; i++)
	{
		...
		for (int j = 0; j < size; j++)
		{
			...
			if (strcmp(type2.c_str(), "nm") == 0) continue;//将第二个字符串保存非nm类型的图片
			if (strcmp(number1.c_str(), number2.c_str()) == 0)
				if (strcmp(angle1.c_str(), angle2.c_str()) == 0)//如果两个字符串角度相等
				{
					...
					//break;
					outfile << files[i].c_str() << " " << stringToNum<int>(number1)-1 << endl;
//					outfile << files[j].c_str() << " " << stringToNum<int>(number1)-1 << endl;
				}
